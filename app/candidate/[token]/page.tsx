...
export default function CandidatePage({ params }: { params: { token: string } }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: r } = await supabase.from('requests').select('*').eq('id', c.request_id).single()
      const { data: reqs } = await supabase.from('request_requirements').select('*').eq('request_id', c.request_id).order('sort_order')
      const { data: old } = await supabase.from('candidate_requirement_scores').select('*').eq('candidate_id', c.id)
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    for (const r of requirements) {
      ...
      await supabase.from('candidate_requirement_scores').upsert({ candidate_id: candidate.id, requirement_id: r.id, score: value.score, evidence: value.evidence }, { onConflict: 'candidate_id,requirement_id' })
    }
    setSent(true)
  }
  ...
  return <main className="min-h-screen bg-slate-50 py-8 px-4" dir="rtl"><div className="max-w-4xl mx-auto">
    ...
    {sent ? <div className="card p-10 text-center"><div className="text-5xl mb-4">✓</div><h2 className="text-2xl font-bold text-[#09233f]">تم إرسال بياناتك بنجاح</h2><p className="text-slate-500 mt-3">شكرًا لك. ستتم مراجعة بياناتك من قبل فريق الموارد البشرية.</p></div> : <form onSubmit={submit} className="space-y-6">
      ...
      <section className="card p-6"><h2 className="section-title">المهارات والمتطلبات</h2><p className="text-sm text-slate-500 mb-5">هذه المتطلبات اختيارية في نموذج المرشح. اختر المستوى الأقرب لخبرتك وأضف توضيحًا عند الحاجة.</p>{Object.entries(grouped).map(([cat, rs]: any) => <div key={cat} className="mb-7"><h3 className="font-bold text-[#09233f] border-b pb-2 mb-3">{cat}</h3>{rs.map((r:any)=><div key={r.id} className="border rounded-xl p-4 mb-3"><div className="font-medium">{r.name}</div><div className="grid md:grid-cols-2 gap-3 mt-3"><select className="input" value={scores[r.id]?.score ?? 0} onChange={e=>setScores({...scores,[r.id]:{score:Number(e.target.value),evidence:scores[r.id]?.evidence||''}})}>{ENGINEERING_LEVELS.map(x=><option key={x.score} value={x.score}>{x.label}</option>)}</select><input className="input" placeholder="اذكر الخبرة أو المشاريع ذات الصلة" value={scores[r.id]?.evidence || ''} onChange={e=>setScores({...scores,[r.id]:{score:scores[r.id]?.score ?? 0,evidence:e.target.value}})}/></div></div>)}</div>)}</section>
