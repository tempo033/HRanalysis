...
export default function NewRequest() {
  ...
  const add = () => setReqs([...reqs, { text: '', weight: 0, category: 'عام', required: false }])

  const addEngineering = () => {
    const existing = new Set(reqs.map(x => x.text.trim()).filter(Boolean))
    const fresh = ENGINEERING_REQUIREMENTS.filter(x => !existing.has(x.name)).map(x => ({ ...x, text: x.name, weight: Number((100 / (reqs.filter(r => r.text.trim()).length + ENGINEERING_REQUIREMENTS.filter(y => !existing.has(y.name)).length)).toFixed(2)), required: false }))
    const all = [...reqs.filter(x => x.text.trim()), ...fresh]
    const equal = Number((100 / all.length).toFixed(2))
    const adjusted = all.map((x, i) => ({ ...x, weight: i === all.length - 1 ? Number((100 - equal * (all.length - 1)).toFixed(2)) : equal }))
    setReqs(adjusted)
  }
  ...
  const submit = async () => {
    ...
    if (total !== 100 || !clean.length) { setError('يجب أن يكون مجموع أوزان المتطلبات 100%.'); return }
    setSaving(true)
  ...
  return <main className="min-h-screen bg-[#f5f7fa]" dir="rtl">
    ...
    <div className="max-w-4xl mx-auto p-5 md:p-10">
      ...
      <div className="card p-6 md:p-8">
        {step===1&&<><h2 className="text-2xl font-bold text-[#09233f]">ما نوع الطلب؟</h2><p className="text-slate-500 mt-2 mb-6">اختر نوع الطلب الذي تريد إنشاؤه.</p><div className="grid sm:grid-cols-2 gap-4">{Object.keys(types).map(t=><button key={t} onClick={()=>{setKind(t);setDetail('')}} className={`text-right p-5 rounded-xl border-2 ${kind===t?'border-[#b88618] bg-amber-50':'border-slate-200 hover:border-slate-300'}`}><b>{t}</b><div className="text-sm text-slate-500 mt-1">طلب {t}</div></button>)}</div></>}
